...
export class AuthService {
    ...
    async register(user: CreateUserDto): Promise<ResponseUserDto> {
        ...
        return new ResponseUserDto({
            status: 201, message: "Usuario creado.",
            id: rsp.id, email: rsp.email
        });
    }

    async login(user: AuthUserDto, response: Response) {
        const userLogin = await this.userService.getUser(user.username);
        if (!userLogin) throw new UnauthorizedException("Credenciales invalidas.");
        
        const password_valid = await bcrypt.compare(user.password, userLogin.password);
        if (!password_valid) throw new UnauthorizedException("Credenciales invalidas.");
